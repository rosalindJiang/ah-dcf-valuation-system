...
```
DCF估值计算系统/
├── app.py              # Web 应用入口（Flask），只做路由和调用，不写业务逻辑
├── config/             # 配置文件，只放参数常量，不放业务逻辑
├── templates/          # Jinja2 + Plotly.js 前端模板
├── data/               # 运行产出（.db），不提交到版本库
├── src/                # 核心业务模块
...
- `templates/index.html` 是项目中唯一使用 Plotly.js 的文件（客户端渲染）
...
| `templates/index.html` | 全栈工程师 | 维护图表类型和 Web 界面样式 |
| `app.py` | 全栈工程师 | 维护 Web 路由和 API 响应逻辑 |
